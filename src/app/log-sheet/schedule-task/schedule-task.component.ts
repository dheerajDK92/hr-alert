...
export class ScheduleTaskComponent implements OnInit, OnChanges, OnDestroy {
  ...
  scheduleTask() {
    ...
    if (
      !this.isNull(this.formData.task) &&
      !this.isNull(this.formData.taskDate) &&
      !this.isNull(this.formData.taskFor)
    ) {
      const data = {
        companyId: this.CmpData._id,
        employeeId: this.empData._id,
        task: this.formData.task._id,
        taskFor: this.formData.taskFor._id,
        taskDate: this.formData.taskDate,
        status:'assigned'
      };
      this.loader.showLoader();
      this._log.scheduleTask(data).subscribe(
        (res) => {
          this.loader.hideLoader();
          console.log("Res===", res);
          this._toast.showWarning(
            `Successfully Task Scheduled To ${this.formData.taskFor.empname} on ${this.formData.taskDate}`
          );
          this.clearFields();
          this.loadScheduleTasks();
        },
      ...
      this._toast.showWarning("Please Select All Fields To Schedule Task");
    }
  }
  /**
   * loadScheduleTasks
  ...
  loadScheduleTasks() {
    ...
    this._log.loadScheduleTask(this.CmpData._id).subscribe(
      (res: any) => {
        this.loader.hideLoader();
        if (res.data.taskDetails?.length > 0) {
